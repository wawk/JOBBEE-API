...
exports.applyJob = catchAsyncErrors( async(req, res, next) => {
    ...
    // Check document file size not greater than 2MB
    if(file.size > process.env.MAX_FILE_SIZE) {
        return next(new ErrorHandler('Please upload file size less than 2MB.'),400);
    }

    // Renaming Resume document
...
res.status(200).json({
    success : true,
    message: 'Application to job was successful - Congratulations!',
    data: file.name
});
});
});
